...
class MST(nn.Module):
    def __init__(self, in_dim=BANDS, out_dim=BANDS, dim=BANDS, stage=2, num_blocks=[2, 4, 4]):
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=4, out_channels=BANDS, n_feat=BANDS, msab_stages=2, stage=3, compressed=True):
        ...
        if compressed:
            ...
            modules_body = [MST(dim=BANDS, stage=2, num_blocks=[1,1,1]) for _ in range(stage)]

        self.body = nn.Sequential(*modules_body)
        self.conv_out = nn.Conv2d(n_feat, out_channels, kernel_size=3, padding=(3 - 1) // 2,bias=False)

        if compressed: self.apply(self._init_weights)
